...
TransformGizmo.prototype.update = function(rotation: THREE.Euler, eye: THREE.Vector3): void {
...
function TransformGizmoTranslate(enableThreeAxisTranslation: boolean): void {
	...
	this.setActivePlane = function(axis: string, eye: THREE.Vector3): void {
...
function TransformGizmoRotate() {
	...
	const CircleGeometry = (radius: number, facing: string, arc: number): THREE.BufferGeometry => {
	...
	this.update = function(_: THREE.Euler, eye2: THREE.Vector3): void {
...
export
function TransformControls(camera: THREE.Camera, domElement: any, enableThreeAxisTranslation: boolean) {
	THREE.Object3D.call(this)
	...
	let oldPositions: Array<THREE.Object3D> = []
	...
	this.attach = function(objects: Array<THREE.Object3D>): void {
	...
	this.setCamera = function(newCamera: THREE.Camera): void {
	...
	this.setMode = function(mode: string): void {
		...
		for (const type in _gizmo)
			if (_gizmo.hasOwnProperty(type)) _gizmo[type].visible = (type === _mode)
	...
	function onPointerDown(event: any): void {
		...
		if (pointer.button === 0 || pointer.button === undefined) {
			...
			if (intersect) {
				...
				if (planeIntersect) {
					oldPositions = scope.objects.map((o: THREE.Object3D) => {
