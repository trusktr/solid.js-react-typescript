...
import {AnnotationManager,getLogger as Logger }from '@mapperai/mapper-annotated-scene'
...
class AnnotationState {
	...
	private saveAnnotations(): Promise<void> {
		...
		return this.saveAnnotationsToFile(savePath, OutputFormat.UTM)
	...
	saveAnnotationsToFile(fileName: string, format: OutputFormat): Promise<void> {
		const annotations = this.annotationManager.allAnnotations()
			.filter(a => a.isValid())

		if (!annotations.length)
			return Promise.reject(Error('failed to save empty set of annotations'))

		if (!this.annotationManager.props.utmCoordinateSystem.hasOrigin && !this.config['output.annotations.debug.allow_annotations_without_utm_origin'])
		...
		return Promise.resolve(mkdirp.sync(dirName))
			.then(() => AsyncFile.writeTextFile(
				fileName,
				JSON.stringify(this.annotationManager.toJSON(format, annotations), null, 2)
			))
			.then(() => this.clean())
	}
}
