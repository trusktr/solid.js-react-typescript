...
export default class App extends React.Component<AppProps, AppState> {
	 ...
	 * Update sessionId
	 */
	private onSessionIdChange = event => this.setState({
	 ...
	 * @param event
	 */
	private onEnvChange = event => this.setState({
	...
	private AnnotatorUI = () => {
		...
		return <React.Fragment>
			...
			<div id="menu_control">
				<button id="status_window_control_btn" className="menu_btn"
				        onClick={this.makeOnStatusWindowClick()}> &#x2139; </button>
	...
	private SetupForm = () => {
		...
		return <form onSubmit={this.startAnnotator}>
			...
			{!isSaffron && <React.Fragment>
				<div>ENV</div>
				<div><input type="text" onChange={this.onEnvChange} value={env} defaultValue="dev"/></div>
			</React.Fragment>}
			
			<div>Session ID</div>
			<div><input id="sessionId" value={sessionId} onChange={this.onSessionIdChange} defaultValue={"58FCDB407765_20180802-171140434"}/></div>
