...
export function S3TileServiceClientFactory(sessionId:string) {
	...
	async function credentialProvider():Promise<IAWSCredentials> {
		...
		if (credentialPromise) {
			...
			if (sessionBucket && (!isNumber(credentials.expiration) || credentials.expiration > Date.now())) {
				return credentials
			}
		}
		...
		credentialPromise = new Promise<IAWSCredentials>(async (resolve, reject) => {
			try {
				...
				const response = (await new SaffronSDK.CloudService.default().makeAPIRequest(
					...
					`device/0/device/${sessionId.substring(0,12)}/session/${sessionId}/credentials`, "annotator")
				).get("data")
