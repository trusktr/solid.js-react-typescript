...
import rygFrontUrl from './ryg_front.png'
import rygBackUrl from './ryg_back.png'
import stopFrontUrl from './stop_front.png'
import stopBackUrl from './stop_back.png'
import yieldFrontUrl from './yield_front.png'
import yieldBackUrl from './yield_back.png'
...
export class TrafficDevice extends Annotation {
	...
	constructor(obj?: TrafficDeviceJsonInputInterface) {
		...
		if (obj) {
			if (obj.markers.length >= this.minimumMarkerCount) {
				...
				if (!this.isValid()) throw Error(`can't load invalid traffic sign with id ${obj.uuid}`)
	...
	setAssociatedMode(position: THREE.Vector3): void {
		...
		if (this.markers.length > 0) this.markers[0].visible = false
	}
