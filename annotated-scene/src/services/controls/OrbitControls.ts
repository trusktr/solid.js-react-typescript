...
export function OrbitControls(object: THREE.Camera, domElement: HTMLCanvasElement): void {
	this.object = object
	...
	this.update = (function(): (updateType: UpdateTypeEnum | null) => boolean {
		...
		return function update(updateType: UpdateTypeEnum | null): boolean {
			...
			if (zoomChanged ||
				...
				if (updateType === UpdateTypeEnum.pan) scope.dispatchEvent(panEvent)
	...
	const panLeft = (function(): (distance: number, objectMatrix: THREE.Matrix4) => void {
	...
	const panUp = (function(): (distance: number, objectMatrix: THREE.Matrix4) => void {
		const v = new THREE.Vector3()
	...
	const pan = (function(): (deltaX: number, deltaY: number) => void {
		const offset = new THREE.Vector3()
	...
	// tslint:disable-next-line:no-empty
	function handleTouchEnd(_: TouchEvent): void {
	}
