...
import {AnnotationSuperTile} from './tiles/AnnotationSuperTile'
...
export class AnnotationManager extends React.Component<IProps, IState> {
	...
	private createAnnotationFromJson(obj: AnnotationJsonInputInterface): [Annotation | null, AnnotationConstructResult] {
		const annotationType = AnnotationType[obj.annotationType]
		const newAnnotation = AnnotationFactory.construct(annotationType, this.props.channel, obj)
	...
	private createAnnotationByType(annotationType: AnnotationType): [Annotation | null, AnnotationConstructResult] {
		const newAnnotation = AnnotationFactory.construct(annotationType, this.props.channel)
	...
	createAndAddAnnotation(
		...
		const result = this.createAnnotationByType(annotationType)
	...
	objectToAnnotations(json: Object): Annotation[] {
		...
		data['annotations'].forEach((obj: AnnotationJsonInputInterface) => {
			const [newAnnotation, result]: [Annotation | null, AnnotationConstructResult] = this.createAnnotationFromJson(obj)
